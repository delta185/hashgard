...
import (
	...
	gapp "github.com/hashgard/hashgard/app"
)
...
func InitializeLCD(nValidators int, initAddrs []sdk.AccAddress, minting bool, portExt ...string) (
	...
	db := dbm.NewMemDB()
	app := gapp.NewHashgardApp(logger, db, nil, true, 0, baseapp.SetPruning(store.PruneNothing))
	cdc = gapp.MakeCodec()
...
func startTM(
	...
	privVal tmtypes.PrivValidator, app *gapp.HashgardApp,
) (*nm.Node, error) {
...
// NOTE: If making updates here also update cmd/hashgard/cmd/hashgardcli/main.go
