...
func TestHashgardCLIMinimumFees(t *testing.T) {
	...
	// start hashgardd server with minimum fees
...
func TestHashgardCLIQueryRewards(t *testing.T) {
	t.Parallel()
	f := InitFixtures(t)
	cdc := app.MakeCodec()
	...
	genFile := filepath.Join(f.HashgarddHome, "config", "genesis.json")
...
func TestHashgardCLIQuerySupply(t *testing.T) {
	t.Parallel()
	f := InitFixtures(t)
...
func TestHashgardCLISubmitProposal(t *testing.T) {
	...
	proposalsQuery = f.QueryGovProposals("--limit=10") //HashGard
	require.Equal(t, uint64(2), proposalsQuery[0].ProposalID)

	f.Cleanup()
}

func TestHashgardCLISubmitParamChangeProposal(t *testing.T) {
	t.Parallel()
	f := InitFixtures(t)
...
func TestHashgardCLISubmitCommunityPoolSpendProposal(t *testing.T) {
	t.Parallel()
	f := InitFixtures(t)
...
func TestHashgardCLIConfig(t *testing.T) {
	...
	config, err := ioutil.ReadFile(path.Join(f.HashgardcliHome, "config", "config.toml"))
	require.NoError(t, err)
