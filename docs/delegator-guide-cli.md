...
This document contains all the necessary information for delegators to interact with the Hashgard Hub through the Command-Line Interface (CLI).
...
Please also note that you are about to interact with the Hashgard Hub, a
...
[Interchain Hashgard Contribution terms](https://github.com/cosmos/cosmos/blob/master/fundraiser/Interchain%20Hashgard%20Contribution%20Terms%20-%20FINAL.pdf)) and that the Interchain Foundation and/or 
...
- [Hashgard Accounts](#cosmos-accounts)
    + [Restoring an Account from the Fundraiser](#restoring-an-account-from-the-fundraiser)
    + [Creating an Account](#creating-an-account)
- [Accessing the Hashgard Hub Network](#accessing-the-cosmos-hub-network)
...
- [Sending Transactions](#sending-transactions)
    ...
    + [Bonding Gards and Withdrawing Rewards](#bonding-gards-and-withdrawing-rewards)
    + [Participating in Governance](#participating-in-governance)
    + [Signing Transactions from an Offline Computer](#signing-transactions-from-an-offline-computer)

## Installing `hashgardcli` 

`hashgardcli`: This is the command-line interface to interact with a `hashgardd` full-node. 

::: warning
**Please check that you download the latest stable release of `hashgardcli` that is available**
:::

[**Download the binaries**](https://github.com/hashgard/hashgard/releases)
...
```bash
hashgardcli keys add <yourKeyName> --recover
```
...
To create an account, you just need to have `hashgardcli` installed. Before creating it, you need to know where you intend to store and interact with your private keys. The best options are to store them in an offline dedicated computer or a ledger device. Storing them on your regular online computer involves more risk, since anyone who infiltrates your computer through the internet could exfiltrate your private keys and steal your funds.
...
```bash
hashgardcli keys add <yourAccountName> --ledger 
```
...
## Accessing the Hashgard Hub Network
...
You will find the tutorial on how to install `hashgardd` [here](https://cosmos.network/docs/cosmos-hub/installation.html), and the guide to run a full-node [here](https://cosmos.network/docs/cosmos-hub/join-mainnet.html).

### Connecting to a Remote Full-Node

If you do not want or cannot run your own node, you can connect to someone else's full-node. You should pick an operator you trust, because a malicious operator could return  incorrect query results or censor your transactions. However, they will never be able to steal your funds, as your private keys are stored locally on your computer or ledger device. Possible options of full-node operators include validators, wallet providers or exchanges. 

In order to connect to the full-node, you will need an address of the following form: `https://77.87.106.33:26657` (*Note: This is a placeholder*). This address has to be communicated by the full-node operator you choose to trust. You will use this address in the [following section](#setting-up-hashgardcli).

## Setting Up `hashgardcli`

::: tip
**Before setting up `hashgardcli`, make sure you have set up a way to [access the Hashgard Hub network](#accessing-the-cosmos-hub-network)**
:::
...
`hashgardcli` is the tool that enables you to interact with the node that runs on the Hashgard Hub network, whether you run it yourself or not. Let us set it up properly.
...
::: tip
**Before you can bond gards and withdraw rewards, you need to [set up `hashgardcli`](#setting-up-hashgardcli)**
:::

`hashgardcli` lets you query all relevant information from the blockchain, like account balances, amount of bonded tokens, outstanding rewards, governance proposals and more. Next is a list of the most useful commands for delegator. 

```bash
// query account balances and other account-related information
hashgardcli query account <yourAddress>
...
::: warning
On Hashgard Hub mainnet, the accepted denom is `ugard`, where `1gard = 1,000,000ugard`
:::
...
::: tip
**Before you can bond gards and withdraw rewards, you need to [set up `hashgardcli`](#setting-up-hashgardcli) and [create an account](#creating-an-account)**
:::
...
// Ex value for parameters (do not actually use these values in your tx!!): <to_address>=gard16m93fezfiezhvnjajzrfyszml8qm92a0w67ntjhd3d0 <amount>=1000000ugard 
// Ex value for flags: <gasPrice>=0.005ugard

hashgardcli tx send <to_address> <amount> --from <yourKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>
```

### Bonding Gards and Withdrawing Rewards
...
// ex value for flags: <validatorAddress>=gardvaloper18thamkhnj9wz8pa4nhnp9rldprgant57pk2m8s, <amountToBound>=10000000ugard, <gasPrice>=0.005ugard

hashgardcli tx staking delegate <validatorAddress> <amountToBond> --from <delegatorKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>
...
// ex value for flags: <stcValidatorAddress>=gardvaloper18thamkhnj9wz8pa4nhnp9rldprgant57pk2m8s, <amountToRedelegate>=100000000ugard, <gasPrice>=0.005ugard

hashgardcli tx staking redelegate <srcValidatorAddress> <destValidatorAddress> <amountToRedelegate> --from <delegatorKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>

// Withdraw all rewards
// ex value for flag: <gasPrice>=0.005ugard

hashgardcli tx distribution withdraw-all-rewards --from <delegatorKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>
...
// ex value for flags: <validatorAddress>=gardvaloper18thamkhnj9wz8pa4nhnp9rldprgant57pk2m8s, <amountToUnbound>=10000000ugard, <gasPrice>=0.005ugard

hashgardcli tx staking unbond <validatorAddress> <amountToUnbond> --from <delegatorKeyName> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice>
```
...
The Hashgard Hub has a built-in governance system that lets bonded Gard holders vote on proposals. There are three types of proposal:
...
::: tip
**Before you can bond gards and withdraw rewards, you need to [bond Gards](#bonding-gards-and-withdrawing-rewards)**
:::
...
hashgardcli tx gov submit-proposal --title "Test Proposal" --description "My awesome proposal" --type <type> --deposit=10000000ugard --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice> --from <delegatorKeyName>

// Increase deposit of a proposal
// Retrieve proposalID from $hashgardcli query gov proposals --status deposit_period
// ex value for parameter: <deposit>=10000000ugard

hashgardcli tx gov deposit <proposalID> <deposit> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice> --from <delegatorKeyName>

// Vote on a proposal
// Retrieve proposalID from $hashgardcli query gov proposals --status voting_period 
// <option>=yes/no/no_with_veto/abstain

hashgardcli tx gov vote <proposalID> <option> --gas auto --gas-adjustment 1.5 --gas-prices <gasPrice> --from <delegatorKeyName>
```
...
```bash
// Bond Gards 
// ex value for flags: <amountToBound>=10000000ugard, <bech32AddressOfValidator>=gardvaloper18thamkhnj9wz8pa4nhnp9rldprgant57pk2m8s, <gasPrice>=0.005ugard, <delegatorAddress>=gard10snjt8dmpr5my0h76xj48ty80uzwhraqalu4eg
