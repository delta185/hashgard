...
::: 提示
加入主网所需的信息(`genesis.json`和种子节点)在[`lauch` repo](https://github.com/hashgard/mainnet/tree/master/latest)中可以找到。
:::
...
::: 警告
如果你想要成为Hashgard Hub主网的验证人，你应该[安全研究](security.md)。
:::
...
::: 警告
这一节内容只针对想要在Hashgard Hub主网启动前就作为初始验证人身份的节点。如果主网已经启动，请跳过这一节。
:::
...
你可以提交你的`gentx`到[launch repository](https://github.com/hashgard/mainnet). 这些`gentx`将会组成最终的genesis.json.
...
你必须要在[区块浏览器](https://explorecosmos.network/validators)中看见你的验证人节点信息。你可以在`~/.hashgardd/config/priv_validator.json`文件中找到`bech32`编码格式的`address`。
...
如果被因为掉线而遭到jail，你可以重获你的投票股权以重回验证人队伍。首先，如果`hashgardd`没有运行，请再次启动：
...
### 问题 #2 : 我的`hashgardd`由于`too many open files`而崩溃

Linux可以打开的默认文件数（每个进程）是1024。已知`hashgardd`可以打开超过1024个文件。这会导致进程崩溃。快速修复运行`ulimit -n 4096`（增加允许的打开文件数）来快速修复，然后使用`hashgardd start`重新启动进程。如果你使用`systemd`或其他进程管理器来启动`hashgardd`，则可能需要在该级别进行一些配置。解决此问题的示例`systemd`文件如下：
